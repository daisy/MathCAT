...
fn silent_intent() -> Result<()> {
    ...
    test("el", "SimpleSpeak", expr, "2 x")?;
    test("el", "LiteralSpeak", expr, "2 x")?;
    return Ok(());
...
fn prefix_intent() -> Result<()> {
    ...
    test("el", "SimpleSpeak", expr, "testing x")?;
    return Ok(());
...
fn postfix_intent() -> Result<()> {
    ...
    test("el", "SimpleSpeak", expr, "x testing")?;
    return Ok(());
...
fn infix_intent() -> Result<()> {
    ...
    test("el", "SimpleSpeak", expr, "x testing y testing z testing 2")?;
    return Ok(());
...
fn function_intent() -> Result<()> {
    ...
    test("el", "SimpleSpeak", expr, "testing του x κόμμα, y κόμμα, z κόμμα, 2")?;
    return Ok(());
...
fn function_one_arg_intent() -> Result<()> {
    ...
    test("el", "SimpleSpeak", expr, "testing του x")?;
    //theodora. fails probably because 
    return Ok(());
...
fn silent_intent_mi() -> Result<()> {
    ...
    test("el", "SimpleSpeak", expr, "2")?;
    test("el", "ClearSpeak", expr, "2")?;
    return Ok(());
...
fn silent_intent_msup() -> Result<()> {
    ...
    test("el", "SimpleSpeak", expr, "κεφαλαίο h 2")?;
    test("el", "ClearSpeak", expr, "κεφαλαίο h 2")?;
    return Ok(());
...
fn intent_prob_x() -> Result<()> {
    ...
    <msup intent='$op($arg)'>
        <mi arg='arg'>x</mi>
        <mi arg='op' intent='πιθανότητα' mathvariant='normal'>P</mi>
    </msup></math>";
    test("el", "ClearSpeak", expr, "πιθανότητα του x")?;
    return Ok(());
