...
fn trace() -> Result<()> {
  ...
  test("fr", "SimpleSpeak", expr, "trace of cap m")?;
  return Ok(());
...
fn dimension() -> Result<()> {
  ...
  test("fr", "SimpleSpeak", expr, "dimension of cap m")?;
  return Ok(());
...
fn homomorphism() -> Result<()> {
  ...
  test("fr", "SimpleSpeak", expr, "homomorphism of cap m")?;
  return Ok(());
...
fn kernel() -> Result<()> {
  ...
  test("fr", "SimpleSpeak", expr, "kernel of cap l")?;
  return Ok(());
...
";
  test("fr", "SimpleSpeak", expr, "norm of f")?;
  return Ok(());
...
";
  test("fr", "SimpleSpeak", expr, "norm of x plus y end norm")?;
  return Ok(());
...
";
  test("fr", "SimpleSpeak", expr, "p norm of f")?;
  return Ok(());
...
"#;
  test("fr", "SimpleSpeak", expr, "open paren, b times nahblah, close paren; times eigh")?;
  return Ok(());
