...
impl BrailleLevel {
    fn from(level: &str) -> BrailleLevel {
        match level {
            "Beginner" => BrailleLevel::Beginner,
            "Advanced" => BrailleLevel::Advanced,
            _ => BrailleLevel::Intermediate,
        }
    }
}
...
static POLISH_INDICATOR_REPLACEMENTS: phf::Map<&str, &str> = phf_map! {
    ...
    "F" => "⠆",      // fraction start indicator
    "f" => "⠰",      // fraction end indicator
    "U" => "⠻",     // unit indicator
...
fn polish_cleanup(_pref_manager: Ref<PreferenceManager>, raw_braille: String) -> String {
    // Repeating decimals can have the form "0,(3)", but the comma rule can't get the comma right (should be number comma) -- fix it here
    static REPEATING_DECIMAL_COMMA: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"(N.)P⠂W(\((N.)+\))").unwrap());
    ...
    // Simple projectors: p22
    // Explanation:
    //  ((>[^⠰])+) -- one or more simple close terminators; they can't contain fractions
    // followed by one of
    //  (<[^⠰]) -- simple opening, but not a fraction start (part c)
    //  ([W>≫/≪])
    //    W - white space (part a)
    //    > - simple closing and fraction close (parts b & d)
    //    ≫ - complex and detailed closing (part b)
    //    / - fraction bar (part d)
    ...
    static REMOVE_SIMPLE_CLOSE_INDICATORS: LazyLock<Regex> = LazyLock::new(|| {
        Regex::new(r"((>[^⠰])+)((<[^⠆])|([W>≫/≪)\]})]))").unwrap()
    });

    // Compound (composite?, complex?) projectors: p23
    // Explanation:
    //  ((≫.])+) -- one or more compound or detailed close terminators
    // followed by one of
    //  ([W≫/≪])
    //    W - white space (part a)
    //    ≫ - complex or detailed closing (part b)
    //    ≪ -- complex or detailed opening (part c)
    //    )\]} -- closing brackets (part d)
    //    (>⠰) - fraction end (part e) -- only valid if in denominator -- FIX: need a separate rule
    static REMOVE_COMPOUND_CLOSE_INDICATORS: LazyLock<Regex> =
    ...
    static REMOVE_CLOSE_INDICATORS_BEFORE_DROP_NUMBERS: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"(>[^⠰])(n.)").unwrap());
    static REMOVE_CLOSE_INDICATORS_AFTER_DROP_NUMBERS: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"(n.)(>[^W⠰])").unwrap());

    // This rule is not in the spec, but discussion with the Polish team said close indicators before commas should go away
    static REMOVE_SCRIPT_CLOSE_INDICATORS_BEFORE_COMMA: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r">⠱P⠂W").unwrap());
    ...
    static SCRIPT_AFTER_DROP_NUMBERS: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"([nN].)p").unwrap());

    // p51: minutes followed by seconds keeps the 'p'
    static MINUTES_SECONDS: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"p⠔(N.+)p⠔⠔").unwrap());

    // "!" is either punctuation or factorial -- we only use the punctuation form when it follows two letters
    static EXCLAMATION_MARK_IS_PUNCTUATION: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"(L.L.)!").unwrap());
    ...
    let result = EXCLAMATION_MARK_IS_PUNCTUATION.replace_all(&result, "${1}P⠖W")
                                                        .replace('!', "⠫W"); // factorial
    debug!("After ! is punctuatin'{}'", &result);
...
#[derive(Debug)]
struct Projectors {
    i_depths: usize,          // index into projector_depths (only increments)
    depths: Vec<usize>,       // depths calculated by max_nesting_depths
    depth_stack: Vec<usize>,  // keep track of current project depth (push on '<', pop on '>')
}

impl fmt::Display for Projectors {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        return write!(f, "depths={:?}, @i={}, depth_stack={:?}", self.depths, self.i_depths, self.depth_stack);
    }
}

impl Projectors {
    fn new(projector_depths: Vec<usize>) -> Projectors {
        return Projectors{
            i_depths: 0,
            depths: projector_depths,
            depth_stack: vec![0],
        }
    }

    fn push(&mut self) -> usize {
                let projector_depth = self.depths[self.i_depths];
                self.depth_stack.push(projector_depth);
                self.i_depths += 1;
                return projector_depth;
    }
...
fn polish_remove_unneeded_mode_changes(raw_braille: &str) -> String {
    ...
    let mut i = 0;
    let braille_level = BrailleLevel::from(&PreferenceManager::get().borrow().pref_to_string("Polish_BrailleLevel"));
    // let braille_level_beginner = braille_level == "Beginner";
    ...
    while i < chars.len() {
        ...
        match ch {
            'l' | 'L' => {
                if unit_mode {
                    ...
                    if letter_mode != new_mode
                        || (mode == BrailleMode::Number
                            && is_letter_number(unhighlight(chars[i + 1])))
            ...
            '𝐿' => {
                ...
                let (end, run): (usize, String) = if braille_level == BrailleLevel::Advanced {chemical_element_run(&chars[i..])} else {(0, String::new())};
                if end > 0 {
                    debug!("end={end}, run='{run}'");
                    if run.is_empty() {
                        result.push_str(chars[i..i+end].iter()
                                    .filter_map(|&c| if c == 'l' {None} else {Some(c)})
            ...
            '>' => {
                // '>' should be at the same level as the matching '<' -- pop after deciding on '>' or '≫'
        ...
        fn previous_was_cap_letter(chars: &[char], i: usize) -> bool {
            static SIMPLE_SCRIPT_MODIFIER_CAP_LETTER: LazyLock<Regex> = LazyLock::new(|| {
                Regex::new(r"L.p(⠔|⠔⠔)").unwrap() // ′ and ″ seem most likely (see Modifiers in definitions.yaml)
            });
            static SIMPLE_SCRIPT_AFTER_CAP_LETTER: LazyLock<Regex> = LazyLock::new(|| {
                Regex::new(r"L.<.[lLnN].>.").unwrap() // something A_1 A_2
            });
    ...
    /// Return the tuple (i_stop, new_str)
    ///   The input string  consists of the char sequence 𝐿x𝐿y... with the possible addition of subscripts ('nd') and parens
    /// If there is a lower case letter, 'new_str' is empty, otherwise it uses the shortened form omitting the cap indicator
    /// 'chars' should start at the first 𝐿
    fn chemical_element_run(chars: &[char]) -> (usize, String) {
        ...
        while i < chars.len() {
            match chars[i] {
                '𝐿' => {n_letters += 1; i += 2;},
                'n' => i += 2,
                'l' => {use_short_form = false; i += 2;},
                // '(' | ')' => i += 1,  // allow parens in chemical formulas
                _ => break,
            }
            run.push(chars[i-1]);   // only meaningful if use_short_form == true
        }
        if !use_short_form || n_letters == 1 {
            run.clear();
        }
        return (i, run);
    }
    ...
    fn max_nesting_depths(chars: &[char], any_start_indicator: bool) -> Vec<usize> {
            ...
            if chars[i] == '<' && (any_start_indicator || chars[i+1] == '⠆') {
                ...
                if braille_level == BrailleLevel::Advanced ||
                   !((any_start_indicator && depths.is_empty() && i - i_start > MAX_CHARS_FOR_INTERMEDIATE) ||
                     has_whitespace_ignoring_fraction_bar(&chars[i_start..i])) {
                    new_depth = depths.iter().copied().max().unwrap_or(0) + 1
                }
                debug!("i_start={i_start}, i={i}, chars='{}', depths={:?}, new_depth={new_depth}",
                        chars[i_start..i].iter().copied().collect::<String>(), depths);
        ...
        match braille_level {
            BrailleLevel::Beginner => {
                // if fractions, move to level 2 so they always have start/end spacing
                // otherwise, if they are all 1, leave alone, otherwise bump to 3 
                if any_start_indicator {
                    if result.iter().any(|&d| d > 1) {
                        result.iter_mut().for_each(|d| *d = 3);
                    }
                } else {
                    result.iter_mut().for_each(|d| *d = 2);
                }
            },
            BrailleLevel::Intermediate => {
                // change the 0's to 3's
                result.iter_mut().for_each(|depth| if *depth==0 {*depth = 3});
            },
            BrailleLevel::Advanced => {}
        }
        return result;
                    ...
                    '<' => {
                        ...
                        *i += 1;
                        if any_start_indicator || chars[*i] == '⠆' {  // fraction start
                            ...
                            debug!("...i_start={i_start}, i={}, chars='{}', depths={:?}, new_depth={new_depth}",
                                    *i, chars[i_start..*i].iter().copied().collect::<String>(), depths);
                            result.push(new_depth);
                            result.extend(depths);
                        }
                    }
        ...
        fn has_whitespace_ignoring_fraction_bar(chars: &[char]) -> bool {
            // Don't need to worry about 'W' being first or last chars
            debug!("has_whitespace_ignoring_fraction_bar: chars='{}', answer={}", chars.iter().copied().collect::<String>(),
